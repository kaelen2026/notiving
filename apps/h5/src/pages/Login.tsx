...
export default function Login() {
  ...
  const handleModeSwitch = useCallback(
    (newMode: LoginMode) => { setMode(newMode); resetForm(); },
    [resetForm],
  );
    ...
    <div className="flex min-h-screen items-center justify-center bg-background px-4">
      <div className="w-full max-w-sm">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-foreground-secondary">Log in to your account</p>
        </div>
        ...
        >
          ...
          {mode === "otp" && step === "code" && (
            <div className="mb-4">
              ...
              <p className="mb-3 text-xs text-foreground-secondary">Sent to {email.trim()}</p>
              ...
              <div className="mt-3 text-center">
                {countdown > 0 ? (
                  <span className="text-xs text-foreground-tertiary">Resend in {countdown}s</span>
                ) : (
                  <button type="button" onClick={handleSendCode} disabled={loading} className="text-xs text-primary hover:text-primary-hover">
                    Resend code
                  </button>
                )}
              </div>
          ...
          <button
            type="submit"
            disabled={loading || !isEmailValid || (mode === "password" && !password) || (mode === "otp" && step === "code" && !isCodeComplete)}
          ...
          >
            {loading ? "Loading..." : mode === "otp" && step === "email" ? "Send Code" : "Log In"}
          </button>
        </form>
      </div>
    </div>
